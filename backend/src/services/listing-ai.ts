...
Return ONLY the title, nothing else. Under 80 characters.`,
  ...
  console.log(
    `[ListingAI/title] model=claude-opus-4-5 input_tokens=${response.usage.input_tokens} output_tokens=${response.usage.output_tokens} latency=${latency}ms`,
  );
...
${pastDescriptions}`,
  ...
  const latency = Date.now() - start;
  console.log(
    `[ListingAI/description] model=claude-opus-4-5 input_tokens=${response.usage.input_tokens} output_tokens=${response.usage.output_tokens} latency=${latency}ms`,
  );
...
Respond ONLY in valid JSON:
{
  ...
  const latency = Date.now() - start;
  console.log(
    `[ListingAI/shipping] model=claude-opus-4-5 input_tokens=${response.usage.input_tokens} output_tokens=${response.usage.output_tokens} latency=${latency}ms`,
  );
